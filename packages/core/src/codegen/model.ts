...
const BINDING_KINDS: ReadonlySet<NodeKind> = new Set([
  'agent', 'pipeline', 'parallel', 'fanout', 'loopUntilCheck',
]);
...
// JS reserved words (+ a few contextual/literal keywords) that can't be a binding
// name. A node labeled `delete`/`class`/`return`/… would otherwise emit
// `const delete = …` and crash the acorn self-lint. Such names are suffixed `_`.
