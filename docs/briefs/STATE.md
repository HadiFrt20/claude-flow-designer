...
M9 | in-progress | Structural view: phase() markers → titled group containers (parentId), agent-gating if → branch nodes w/ verbatim condExpr (top-level + nested). Corpus: 176 phase() + 7 orchestration-if surfaced (was 0/0). Round-trip: byte-identical for phase groups + all M6–M8 fixtures; fixpoint (best-effort) for branch regions
