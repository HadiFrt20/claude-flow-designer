...
| `fanout` | `const <lhs> = await <call>([ <branch>, … ])` where `<call>` is `parallel` (mode `parallel`) or `Promise.all` (mode `promiseAll`), and `<lhs>` is the verbatim `bindingPattern` when set (`[a, b]`) else the derived name. Each `<branch>` is `...<sourceExpr>.map(<itemVar> => () => agent(<promptArg>, {…}))` for a `map` branch or `() => agent(<promptArg>, {…})` for a `thunk` branch, emitted in `branches` order |
